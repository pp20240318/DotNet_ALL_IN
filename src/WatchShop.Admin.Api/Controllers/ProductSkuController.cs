...
public class ProductSkuController : ApiControllerBase
{
    ...
    public async Task<IActionResult> GetByProductId(long productId)
        => Success(await _productSkuService.GetByProductIdAsync(productId));

    [HttpGet("{id:long}")]
    public async Task<IActionResult> GetById(long id)
    {
        var item = await _productSkuService.GetByIdAsync(id);
    ...
    public async Task<IActionResult> Create([FromBody] ProductSkuCreateRequest request)
        => Success(new { id = await _productSkuService.CreateAsync(request) }, "创建成功");

    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update(long id, [FromBody] ProductSkuUpdateRequest request)
    {
        await _productSkuService.UpdateAsync(id, request);
    ...
    {
        await _productSkuService.StockInAsync(id, quantity);
    ...
    {
        await _productSkuService.StockOutAsync(id, quantity);
